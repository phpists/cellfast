...
<div class="row justify-content-between">
    ...
    <div class="col like-box">
		<?= AdminHelper::getStatusWidget($form, $model, null, \common\models\Article::STATUS_ACTIVE)?>
    </div>
</div>
			...
			'clientOptions' => [
				...
				'create' => new \yii\web\JsExpression('function(input, callback) {
                        $.ajax({
                            ...
                            data: {project: $(\'#article-project_id\').val(), value: input},
                            type: \'post\',
                            dataType: \'json\',
		...
		<?php foreach (Yii::$app->languages->languages as $language) :?>
			<?= $form->field($model, "teaser_{$language->suffix}")->widget(Widget::className(), [
				'settings' => [
					...
					'imageUpload' => Url::to(['article/body-image-upload']),
					'imageManagerJson' => Url::to(['article/body-images-get']),
					'imageDelete' => Url::to(['article/body-file-delete']),
					'plugins' => [
						'fullscreen',
						'table',
						'video',
