<?php
/**
 * Временная модель (товары с noit.me)
 */
...
class ProductMe extends BaseContent
{
	/**
	 * Entity-model class
	 */
	...
	{
		return $this->hasMany($this->itemModelClass, ['product_id' => 'id'])->orderBy([AdminHelper::FIELDNAME_SORT => SORT_ASC]);
	}
...
//			->andWhere(['!=', 'pi.status', ProductItem::STATUS_UNAVAILABLE]);
	...
	{
		return self::find()->where(['sku' => trim($sku)])->one();
	}
}
