...
class SerializedAttributes extends \baibaratsky\yii\behaviors\model\SerializedAttributes
{
	/**
	...
	public function deserializeAttributes()
	{
		foreach ($this->attributes as $attribute) {
			...
			/** TODO */
			if (isset($this->owner->{$attribute}[0]) && empty($this->owner->{$attribute}[0])) {
				$this->owner->{$attribute} = [];
			} else {
				$this->owner->{$attribute} = array_values($this->owner->{$attribute});
			}
